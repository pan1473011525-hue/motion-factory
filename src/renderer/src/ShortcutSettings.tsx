...
export const ShortcutSettings: React.FC<{
...
}> = ({open, bindings, platform, onClose, onChange}) => {
  const [query, setQuery] = useState("");
  const [recording, setRecording] = useState<ShortcutCommandId | null>(null);
  const [conflict, setConflict] = useState<string | null>(null);
  const defaults = useMemo(() => getDefaultShortcutBindings(platform), [platform]);
  ...
  return <section className="shortcut-settings-popover" aria-label="键盘快捷键设置">
    ...
    <div className="shortcut-settings-tools"><input type="search" placeholder="搜索命令" value={query} onChange={(event) => setQuery(event.target.value)} /><button type="button" onClick={() => {onChange(structuredClone(defaults)); setConflict(null);}}><RotateCcw />全部恢复默认</button></div>
