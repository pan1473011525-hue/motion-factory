...
export const HorizontalRanking: React.FC<HorizontalRankingProps> = (props) => {
  const frame = useCurrentFrame(); const {durationInFrames} = useVideoConfig(); const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor);
...
export const VerticalBars: React.FC<VerticalBarsProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const draw = useDrawProgress();
...
export const LineTrend: React.FC<LineTrendProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const progress = useDrawProgress();
...
export const DonutShare: React.FC<DonutShareProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const progress = useDrawProgress(6, 66); const total = Math.max(1, props.items.reduce((sum, item) => sum + Math.max(0, item.value), 0));
  let angle = 0;
...
export const SourceCard: React.FC<SourceCardProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor);
...
export const Timeline: React.FC<TimelineProps> = (props) => {
  const frame = useCurrentFrame(); const {durationInFrames} = useVideoConfig(); const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const pagination = getPagination(frame, durationInFrames, props.items.length, 4); const items = props.items.slice(pagination.page * 4, pagination.page * 4 + 4);
...
export const LowerThird: React.FC<LowerThirdProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const alignRight = props.position === "right";
...
export const MediaInfo: React.FC<MediaInfoProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const full = props.layout === "media-full"; const reverse = props.layout === "media-right"; const mediaProps = {fit: props.mediaFit, focalX: props.focalX, focalY: props.focalY, scale: props.mediaScale, radius: props.mediaRadius * unit, inSeconds: props.videoInSeconds, outSeconds: props.videoOutSeconds, playbackRate: props.playbackRate, playbackMode: props.playbackMode};
...
export const MediaGrid: React.FC<MediaGridProps> = (props) => {
  const unit = useCanvasUnit(); const theme = getMotionTheme(props.stylePreset, props.accentColor); const slots = [{asset: props.asset1, label: props.label1}, {asset: props.asset2, label: props.label2}, {asset: props.asset3, label: props.label3}, {asset: props.asset4, label: props.label4}].slice(0, props.gridCount);
